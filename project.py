...
def loadStutree(treev):
	...
	if os.path.isfile("Student.json"):
		...
		stu={"Students":[{"Rollno":'',"Name":'',"Gender":'',"Phone no":'',"address":'','Batch':'','Hostel':''}]}
	for i in stu['Students']:
	 	treev.insert("", 'end', text ="", values =(i["Rollno"], i["Name"], i["Gender"],i['address'],i['Phone no'],i['Batch'],i['Hostel'])) 
...
e11=ttk.Combobox(tab5, width = 27, textvariable = n)
if os.path.isfile('Course.json'):
	with open("Course.json") as f:
		cr=json.load(f)
		for i in cr['Courses']:
...
load()
loadcourses()
btn13=Button(tab2,text="Show Students",bg='black',fg='white',font=('Biome',11),command=lambda:loadStutree(treev),width=15)
...
btn14=Button(tab4,text="Show Courses",bg='black',fg='white',font=('Biome',11),command=lambda:loadCoutree(treevc),width=15)
...
################################################################################
info_lbl=Label(root,text='Department of Computer Science & Engineering',width=900, height=150,font=('Impact',12),bg='Black',fg='White')
